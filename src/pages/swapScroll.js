...
const pickDex = type => {
  ...
  switch (type) {
    case "randomSwap":
      dex = pickRandomArrayEl(Object.keys(settings.directSwap.scrollMainnet))
      callback = sendTx[dex]
      break
    case "randomSwapReverse":
      dex = pickRandomArrayEl(Object.keys(settings.reverseSwap.scrollMainnet))
      callback = sendTx[dex]
      break
    default:
...
function SwapScroll() {
  ...
  const submitTx = async (row, i, isRetry = false) => {
    ...
    try {
      const result = await callback({privateKey: row.privateKey, network: "scrollMainnet"})
      if (result.success) {
  ...
  return (
    <>
      <h1 className="mt-5 mb-3">Scroll Tx automation</h1>
      ...
      >
        {[
          "randomSwap",
          "randomSwapReverse",
          ...Object.keys(settings.directSwap.scrollMainnet),
          ...Object.keys(settings.reverseSwap.scrollMainnet),
          ...
          <option value={el} key={el}>
            {el === "randomSwap"
              ...
              : {
                  ...settings.directSwap.scrollMainnet,
                  ...settings.reverseSwap.scrollMainnet,
      ...
      <div className="row mb-5">
        <Table data={data} type={type} onRetry={submitTx} chain="scrollMainnet"></Table>
      </div>
      <div className="row mb-5">
